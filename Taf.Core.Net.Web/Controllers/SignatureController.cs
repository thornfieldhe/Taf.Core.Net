...
public class SignatureController : ControllerBase{
    ...
    public async Task<IActionResult> Get(string appId, string? userId, string timestamp, string sign, string? callback){
        try{
            ...
            if(!string.IsNullOrWhiteSpace(shortUrl.UserId)){
                await _shortUrlService.CreatUserCallingInfo(new UserWithShortCodeDto(){
                    AppId          = shortUrl.AppId
                  , CreationTime   = date
                  , ExpirationDate = shortUrl.ExpiraionDate
                  , ShortCode      = targeturl.ShortCode
            ...
            return Redirect(targeturl.TargetUrl);
        } catch(Exception ex){
