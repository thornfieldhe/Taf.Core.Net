...
public partial class ShortLinks : ComponentBase{
    [Inject]
    public  IShortUrlService ShortUrlService{ get; set; }
    ...
    private Task<PagedResultDto<ShortUrlListDto>> GetAllList(string key = null) => ShortUrlService.GetAllList(new BaseQueryRequestDto(){ KeyWord = key });
  ...
  private  async Task<PagedResultDto<ShortUrlListDto>> QueryAll(string keyWord, string shorting, int index) =>
      await ShortUrlService.GetAllList(new BaseQueryRequestDto(){
